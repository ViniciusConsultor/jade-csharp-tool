...
namespace Jade.CQA.Robot
{
    public partial class Crawler : DisposableBase
    {
        ...
        private void EndDownload(RequestState<ThreadSafeCounter.ThreadSafeCounterCookie> requestState)
        {
            ...
            using (requestState.State)
            {
                if (IsUserProxy && requestState.Request.Proxy != null)
                {
                    ...
                    {
                        if (requestState.DownloadTimer.Elapsed.TotalSeconds < 10)
                        {
                            m_Logger.Verbose("Proxy {0} End Elapsed:{1}s", ((WebProxy)requestState.Request.Proxy).Address, requestState.DownloadTimer.Elapsed.TotalSeconds);
                            ProxyEnd((WebProxy)requestState.Request.Proxy);
                        }
                        else
                        {
                            m_Logger.Verbose("{0} 被废弃,下载耗时{1}s", ((WebProxy)requestState.Request.Proxy).Address.AbsoluteUri, requestState.DownloadTimer.Elapsed.TotalSeconds);
                            Console.WriteLine(((WebProxy)requestState.Request.Proxy).Address.AbsoluteUri + "被废弃,下载耗时" + requestState.DownloadTimer.Elapsed.TotalSeconds);
                        }
                    }
                }
        ...
        public List<WebProxy> Proxyes { get; set; }



        void ProxyEnd(WebProxy proxy)
        {
            if (Proxyes.IsNotNull())
            {
                Proxyes.Add(proxy);
            }
        }
        ...
        WebProxy GetWebProxy()
        {
            ...
            {
                if (Proxyes.IsNotNull() && Proxyes.Count > 0)
                {
                    var proxy = Proxyes[0];
                    Proxyes.RemoveAt(0);
                    m_Logger.Verbose("GetWebProxy()  => {0},Left:{1} proxy", proxy.Address, Proxyes.Count - 1);
                    return proxy;
                }
            }
            return null;
        ...
        private void StartDownload()
        {
            ...
            webDownloader.Proxy = GetWebProxy();
            m_Logger.Verbose("Start Downloading {0}", crawlerQueueEntry.CrawlStep.Uri);
