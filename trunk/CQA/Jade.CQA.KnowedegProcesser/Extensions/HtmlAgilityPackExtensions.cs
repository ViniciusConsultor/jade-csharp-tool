...
namespace Jade.CQA.KnowedegProcesser.Extensions
{
    public static class HtmlToText
    {
        ...
        public static string ExtractData(this HtmlDocument htmlDocument, string xpath)
        {
            ...
            if (!nodes.IsNull())
            {
                return string.Join(" | ", (
                     from entry in nodes
                     select entry.InnerText).ToArray());
            }
            return "";
        }

        public static string ExtractDataById(this HtmlDocument htmlDocument, string id)
        {
            var nodes = htmlDocument.DocumentNode.SelectNodes("//*[@id='" + id + "']");
            if (!nodes.IsNull())
            {
                return string.Join(" | ", (
                     from entry in nodes
